 ...
 * @api public
 */
const CAS = module.exports = function CAS(options) {
    options = options || {};
    ...
    if (options.external_pgt_url) {
        const pgt_url = url.parse(options.external_pgt_url);
        ...
        if (options.external_proxy_url) {
            const proxy_url = url.parse(options.external_proxy_url);
...
CAS.prototype.authenticate = function (req, res, callback, service) {
    const casURL = 'https://' + this.hostname + ':' + this.port + this.base_path;
    ...
    if (!ticket) {
        ...
        const redirectURL = casURL + '/login?service=' + encodeURIComponent(service);
...
CAS.prototype.handleSingleSignout = function (req, res, next, logoutCallback) {
    if (req.method === 'POST' && req.body['logoutRequest']) {
        ...
        try {
            ...
            const $ = cheerio.load(req.body['logoutRequest']);
            const ticketElems = $('samlp\\:SessionIndex');
            if (ticketElems && ticketElems.length > 0) {
                ...
                const ticket = ticketElems.first().text().trim();
...
CAS.prototype.logout = function (req, res, returnUrl, doRedirect) {
    ...
    const redirectURL = 'https://' + this.hostname + ':' + this.port + this.base_path + logout_path;
    res.writeHead(307, {'Location': redirectURL});
...
CAS.prototype.validate = function (ticket, callback, service, renew) {
    ...
    let validate_path;
    let pgtURL;
    const cas_version = this.version;
    if (this.version < 2.0) {
        // CAS 1.0
    ...
    // initialization.
    let service_url = service || this.service;
    if (!service_url) {
    ...
    const queryPath = url.format({
        pathname: this.base_path + (this.base_path[this.base_path.length - 1] !== '/' ? '/' : '') + validate_path,
        query: query
    });


    function flattenXmlJson(json) {
        const result = {};
        const attributes = {};
        _.each(json, (v, k) => {
            if (k === "$") {
                // check if attributes
                _.each(v, (attrValue, attrKey) => {
                    // attributes
            ...
            // attributes
            result[k] = (_.isObject(item) ? flattenXmlJson(item) : item);
        });
        ...
        return {value: _.isEmpty(value) ? undefined : value, attributes: attributes};
    }
        ...
        return new Promise((resolve, reject) => {
            parseString(xml, function (err, json) {
                if (err) {
                    ...
                    // replace all Array[] by its first element
                    resolve(flattenXmlJson(json));
                }
            });
    ...
    const req = https.get({
        ...
        res.on('end', async () => {
            ...
            // CAS 2.0 (XML response, and extended attributes)
                ...
                const responseJs = await xml2json(response);
                // Check for auth success

                const casResponse = responseJs.value["cas:serviceResponse"];
                if (!casResponse) {
                    callback(new Error("No CAS Response"), false);
                    return;
                }

                const casSuccess = casResponse.value["cas:authenticationSuccess"];

                if (casSuccess) {

                    const casUser = casSuccess.value["cas:user"];
                    if (!casUser) {
                    ...
                    let pgtIOU;
                    const casPGT = casSuccess.value["cas:proxyGrantingTicket"];
                    if (casPGT) {
                        pgtIOU = casPGT;
                    }
                    ...
                    const proxies = [];
                    const casProxies = casSuccess.value["cas:proxies"];
                    if (casProxies && Array.isArray(casProxies)) {
                        for (let i = 0; i < casProxies.length; i++) {
                            const thisProxy = casProxies[i].trim();
                            proxies.push(thisProxy);
                        }
                    }

                    // Look for optional attributes
                    const attributes = parseAttributes(casSuccess);

                    callback(undefined, true, casUser, {
                        'username': casUser,
                ...
                const casFailure = casResponse.value["cas:authenticationFailure"];

                // Check for correctly formatted auth failure message
                if (casFailure) {
                    const code = casFailure.attributes.code;
                    let message = 'Validation failed [' + code + ']: ';
                    message += casFailure.value;
...
CAS.prototype.getProxyGrantingTicket = function (pgtIOU, callback) {
    ...
    if (this.is_pgt_external) {
        const urlFetchPGT = url.parse(this.pgt_url + 'getPGT?pgtiou=' + pgtIOU);
        ...
        const req = https.get(urlFetchPGT, function (res) {
            res.on('data', function (chunk) {
                pgt += chunk;
                if (pgt.length > 1e6) {
                    req.connection.destroy();
    ...
    this.getProxyGrantingTicket(pgtIOU, function (err, pgt) {
        ...
        const req = https.get({
            ...
            res.on('end', function () {
                ...
                const elemTicket = $('cas\\:proxyTicket').first();
                if (elemTicket && elemTicket.length > 0) {
                    const proxyTicket = elemTicket.text();
                    callback(undefined, proxyTicket);
                    return;
                }
                // Got a proxy failure
                const elemFailure = $('cas\\:proxyFailure').first();
                if (elemFailure && elemFailure.length > 0) {
                    const code = elemFailure.attr('code');
                    let message = 'Proxy failure [' + code + ']: ';
...
CAS.prototype.startPgtServer = function (key, cert, ca, callbackHost, callbackPort, proxyPort) {
    ...
    // the CAS server.
    const pgtServer = https.createServer(serverOptions);
    ...
    pgtServer.addListener("request", function (req, res) {
        ...
        if (reqURL.pathname === '/getPGT') {
            const pgtIOU = reqURL.query['pgtiou'];
        ...
        // are. It expects only a HTTP 200 response in return.
            ...
            const pgtIOU = reqURL.query['pgtIou'];
            const pgtID = reqURL.query['pgtId'];
            if (pgtIOU && pgtID) {
    ...
    this.pgtInterval = setInterval(function () {
        ...
        for (let pgtIOU in self.pgtStore) {
            const timestamp = self.pgtStore[pgtIOU]['time'];
    ...
    if (proxyPort) {
        const proxyServer = https.createServer(serverOptions);
        ...
        proxyServer.addListener("request", function (req, res) {
            ...
            try {
                const pgtIOU = req.headers['cas-proxy-pgtiou'];
                if (!pgtIOU) {
                ...
                let targetURL = req.headers['cas-proxy-targeturl'];
                if (!targetURL) {
                    throw 'Header "cas-proxy-targeturl" was not found';
                }
                const targetOptions = url.parse(targetURL, true);
...
CAS.prototype.proxiedRequest = function (pgtIOU, options, callback) {
    ...
    const targetService = url.format(options);
    ...
    this.getProxyTicket(pgtIOU, targetService, function (err, pt) {
        ...
        const path = options.path || targetService.replace(/^https?:\/\/[^\/]+/, '');
        if (path.match(/[&?]/)) {
        ...
        try {
            switch (options.method.toUpperCase()) {
                ...
                case 'HEADER': {
                    ...
                    const req = serviceObj.get(options, function (res) {
                ...
                case 'PUT': {
                    ...
                    let res = undefined;
                    const req = serviceObj.request(options, function (_res) {
                        res = _res;
                    });
                    callback(undefined, req, res);
                    break;
...
CAS.prototype.proxiedRequestExternal = function (proxyURL, pgtIOU, options, callback) {
    const targetService = url.format(options);
    const proxyInfo = url.parse(proxyURL);
    ...
    try {
        const req = serviceObj.get(proxyInfo, function (res) {
            callback(undefined, req, res);
        });
    } catch (e) {
        callback(e);
 ...
 */
const getAttributesValues = function (casAttributes) {
    const attributes = {};
    _.each(casAttributes.value, (attrValue, k) => {
        const attrName = k.toLowerCase().replace(/cas:/, '');
        let value = attrValue;
        if (_.has(attrValue, "value") && _.has(attrValue, "attributes")) {
            value = _.isObject(attrValue.value) ? getAttributesValues(attrValue) : attrValue.value;
        }
...
const parseAttributes = function (casSuccess) {
    let attributes = {};
    const casAttributes = casSuccess.value["cas:attributes"];
    if (casAttributes) {
        ...
        //              <cas:userAttributes>
        //                  <lastLogin xmlns="">2018-07-05T21:01:10.973+02:00</lastLogin>
        ...
        //  </cas:serviceResponse>
        //
        attributes = getAttributesValues(casAttributes);
    } else {
        ...
        //    </cas:serviceResponse>
        //
        _.each(casSuccess, (attrValue, k) => {
            let tagName = k.toLowerCase().replace(/cas:/, '');
            ...
            switch (tagName) {
                ...
                case '#text':
                    ...
                    break;
                case 'attribute':
                    if (attrValue.attributes && attrValue.attributes.name) {
                        tagName = attrValue.attributes.name;
                        value = attrValue.attributes.value;
                    }
                default:
                    if (attributes[tagName]) {
                        attributes[tagName] = [attributes[tagName]];
                        attributes[tagName].push(attrValue.value);
                    } else {
                        attributes[tagName] = attrValue.value;
                    }
                    break;
            }
        });

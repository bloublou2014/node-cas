  ...
  const req = https.get({
    ...
    res.on('end', function () {
      ...
      // CAS 2.0 (XML response, and extended attributes)
        ...
        const elemSuccess = $('cas\\:authenticationSuccess').first();
        if (elemSuccess && elemSuccess.length > 0) {
          ...
          for (let i = 0; i < elemProxies.length; i++) {
            const thisProxy = $(elemProxies[i]).text().trim();
            proxies.push(thisProxy);
          }

          // Look for optional attributes
          const attributes = parseAttributes(elemSuccess);
        ...
        const elemFailure = $('cas\\:authenticationFailure').first();
 ...
 */
const parseAttributes = function (elemSuccess) {
  const attributes = {};
  ...
  if (elemAttribute && elemAttribute.children().length > 0) {
    ...
    for (let i = 0; i < elemAttribute.children().length; i++) {
      ...
      if (attrName !== '#text') {
        if (node.name.indexOf("userattributes") < 0) {
          ...
          // go throw all paramters
          const nb = node.children.length;
          for (let j = 0; j < nb; j++) {
            const childNode = node.children[j];
            if (childNode.name && childNode.type !== 'text') {
              const childAttrName = childNode.name.toLowerCase().replace(/cas:/, '');
              const nodeAttrValue = cheerio(childNode).text();
              if (attributes[attrName][childAttrName]) {
                attributes[attrName][childAttrName] = [attributes[attrName][childAttrName]];
                attributes[attrName][childAttrName].push(nodeAttrValue);
              } else {
                attributes[attrName][childAttrName] = nodeAttrValue;
              }
            }
          }
        }
  ...
  if (attributes === {}) {
    ...
    if (nodes && nodes.length) {
      for (let  i = 0; i < nodes.length; i++) {
        const attrName = nodes[i].attr('name');
        const attrValue = nodes[i].attr('value');
        if (!attributes[attrName]) {
          attributes[attrName] = [attrValue];
        } else {
          attributes[attrName].push(attrValue);
